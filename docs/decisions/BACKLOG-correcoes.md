...
| 31/08/2026 | `server.py` lê `MOODLE_TOKEN`/`MOODLE_URL` de `os.environ` e nada carrega o `.env` para ele — o conserto do `conftest` cobre só a suíte. Rodando por um cliente MCP o token tem de vir do `env` da configuração; rodando à mão, falha. Decidir se há um carregador compartilhado (`usp_mcp/env.py`) ou se o entrypoint depende do cliente. | `usp_mcp/moodle/server.py` | média | 🔵 aberto |
...
| 31/08/2026 | O `.venv` não existe no repo nem em worktree novo, e o `.env` também não (gitignorado, não copiado por `git worktree add`). Decidir se vira `requirements-dev.txt` + nota no README. | repo | baixa | 🔵 aberto |
